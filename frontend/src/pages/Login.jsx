...
export default function Login() {
  ...
  const handleRegister = async (e) => {
    e.preventDefault();
    if (reg.senha.length < 6) { setError('Senha deve ter pelo menos 6 caracteres.'); return; }
  ...
  return (
    <div className="auth-view">
      <div className="auth-copy">
        <div style={{ width:64, height:64, background:'var(--accent)', borderRadius:14, display:'flex', alignItems:'center', justifyContent:'center', fontSize:32, marginBottom:20 }}>📋</div>
          ...
          <div className="auth-form">
            ...
            <form onSubmit={handleRegister}>
              <label>Nome e sobrenome *
                <input value={reg.nome} onChange={e => r('nome', e.target.value)} placeholder="Nome completo" required />
              </label>
              <label>CNPJ do prestador
                <input
                  value={reg.cnpj}
                  onChange={e => r('cnpj', maskCNPJ(e.target.value))}
              ...
              <label>Area de atuacao
                <select value={reg.area_atuacao} onChange={e => r('area_atuacao', e.target.value)}>
              ...
              <label>E-mail *
                <input type="email" value={reg.email} onChange={e => r('email', e.target.value)} placeholder="seu@email.com" required />
              </label>
              <label>Telefone
                <input
                  value={reg.telefone}
                  onChange={e => r('telefone', maskTelefone(e.target.value))}
            ...
            <p className="form-note">Se o CNPJ ja estiver na base oficial, o nome sera preenchido automaticamente.</p>
          ...
          <div className="auth-form">
            ...
            <form onSubmit={handleRecover}>
              <label>E-mail cadastrado
                <input type="email" value={recEmail} onChange={e => setRecEmail(e.target.value)} placeholder="seu@email.com" required />
              </label>
              {error && <div className="error-message">⚠ {error}</div>}
              {success && <div style={{ color:'var(--green)', fontSize:13, marginTop:8 }}>✓ {success}</div>}
